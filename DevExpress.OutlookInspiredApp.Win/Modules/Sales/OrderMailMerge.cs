namespace DevExpress.OutlookInspiredApp.Win.Modules {
    ...
    public partial class OrderMailMerge : BaseModuleControl, IRibbonModule {
        public OrderMailMerge()
            : base(CreateViewModel<OrderMailMergeViewModel>) {
            InitializeComponent();
            BindCommands();
            UpdateUI();
            //
        ...
        void ShowReport() {
            ...
            switch(ViewModel.MailTemplate.GetValueOrDefault()) {
                case SalesReportType.SalesReport:
                    snapControl.DataSource = ViewModel.GetOrderItems();
                    ShowReportByMonth(GetSalesReportPeriod());
                    break;
                case SalesReportType.SalesByStore:
                    snapControl.DataSource = ViewModel.GetOrderItems(CollectionViewModel.SelectedEntity.StoreId);
                    GenerateReport(null);
                    break;
